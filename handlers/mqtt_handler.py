...
class MQTTHandler:
    ...
    @classmethod
    def get_client(cls, client_id : str, subscribe_topics : str | list[str]) -> mqtt.Client:
    ...
    def on_connect(client : mqtt.Client, userdata : dict[str, Any], flags, reason_code : ReasonCode, properties) -> None:
        ...
        if connection_result == "succeeded":
            ...
            subscriptions = (topics, 2) if not isinstance(topics, list) else [(topic, 2) for topic in topics]

            print(f"[{client._client_id.upper()}]: Subscribing to {topics if not isinstance(topics, list) else ", ".join(topics)}...")
            client.subscribe(
                topic = subscriptions,
            )
    ...
    def on_subscribe(client : mqtt.Client, userdata : dict[str, Any], mid, reason_code_list : list[ReasonCode], properties) -> None:
        ...
        topics = userdata["subscribed_topics"] if isinstance(userdata["subscribed_topics"], list) else [userdata["subscribed_topics"]]
